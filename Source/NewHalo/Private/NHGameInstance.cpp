...
UNHGameInstance::UNHGameInstance()
{
...
void UNHGameInstance::Init()
{
	Super::Init();

	GetWorld()->GetTimerManager().SetTimer(GetResponseTimeHandle, this, &UNHGameInstance::GetResponseTime, 1.0f, true,
	                                       1.0f);
}

void UNHGameInstance::SetCognitoTokens(FString NewAccessToken, FString NewIdToken, FString NewRefreshToken)
{
	...
	GetWorld()->GetTimerManager().SetTimer(RetrieveNewTokensHandle, this, &UNHGameInstance::RetrieveNewTokens, 1.0f,
	                                       false, 3300.0f);
}

void UNHGameInstance::RetrieveNewTokens()
{
	if (AccessToken.Len() > 0 && RefreshToken.Len() > 0)
	{
		...
		if (FJsonSerializer::Serialize(RequestObj.ToSharedRef(), Writer))
		{
			...
			RetrieveNewTokensRequest->OnProcessRequestComplete().BindUObject(
				this, &UNHGameInstance::OnRetrieveNewTokensResponseReceived);
		...
		{
			GetWorld()->GetTimerManager().SetTimer(RetrieveNewTokensHandle, this, &UNHGameInstance::RetrieveNewTokens,
			                                       1.0f, false, 30.0f);
		}
	}
}

void UNHGameInstance::OnRetrieveNewTokensResponseReceived(FHttpRequestPtr Request, FHttpResponsePtr Response,
                                                          bool bWasSuccessful)
{
	if (bWasSuccessful)
	{
		...
		if (FJsonSerializer::Deserialize(Reader, JsonObject))
		{
			...
			{
				SetCognitoTokens(JsonObject->GetStringField("accessToken"), JsonObject->GetStringField("idToken"),
				                 RefreshToken);
			}
		}
		else
...
void UNHGameInstance::GetResponseTime()
{
	auto GetResponseTimeRequest = HttpModule->CreateRequest();
	GetResponseTimeRequest->OnProcessRequestComplete().BindUObject(
		this, &UNHGameInstance::OnGetResponseTimeResponseReceived);
...
void UNHGameInstance::OnGetResponseTimeResponseReceived(FHttpRequestPtr Request, FHttpResponsePtr Response,
                                                        bool bWasSuccessful)
{
