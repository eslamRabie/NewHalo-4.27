        ...
        static pbuffer::DescribePlayerSessionsRequest* ParseFromDescribePlayerSessionsRequest(const DescribePlayerSessionsRequest &describePlayerSessionsRequest)
        {
            ...
            return event;
...
#else
					static void ParseFromBufferedDescribePlayerSessionsResponse(
						const pbuffer::DescribePlayerSessionsResponse& describePlayerSessionsResponse,
						DescribePlayerSessionsResult& result)
					{
						...
						for (int i = 0; i < describePlayerSessionsResponse.playersessions_size(); i++)
						{
							PlayerSession playerSession = ParseFromBufferedPlayerSession(
								describePlayerSessionsResponse.playersessions(i));
							result.AddPlayerSession(playerSession);
						}
					}

					static pbuffer::DescribePlayerSessionsRequest* ParseFromDescribePlayerSessionsRequest(
						const DescribePlayerSessionsRequest& describePlayerSessionsRequest)
					{
						pbuffer::DescribePlayerSessionsRequest* event = new pbuffer::DescribePlayerSessionsRequest;
						...
						{
							event->set_playersessionstatusfilter(
								describePlayerSessionsRequest.GetPlayerSessionStatusFilter());
						}
				...
				private:
					static PlayerSession ParseFromBufferedPlayerSession(const pbuffer::PlayerSession& bPlayerSession)
					{
						...
						PlayerSessionStatus status = PlayerSessionStatusMapper::GetPlayerSessionStatusForName(
							bPlayerSession.status().c_str());
